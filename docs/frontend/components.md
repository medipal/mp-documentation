...
│       ├── ToggleRenderer.vue
│       ├── VaultReferenceRenderer.vue  Vault picker popover for plugin config
│       └── VerticalLayoutRenderer.vue
├── Modals/
...
│   ├── Patient/
│   ├── Plugin/
│   │   ├── CreatePluginInstanceModal.vue   Create/edit plugin instance + config form
│   │   ├── PluginActionsModal.vue          Browse plugin definition actions
│   │   └── PluginInvokeModal.vue           Invoke a plugin action with inputs
│   ├── Questionnaire/
...
- `CreatePluginInstanceModal` — plugin instance config using `instance_config_schema`

Custom renderers override all default `@jsonforms/vue-vanilla` renderers. See `app/utils/jsonFormRenderer.ts` for the renderer registry (15 renderers) and priority order.

For plugin config forms, the `VaultReferenceRenderer` (rank 9) enables a vault picker popover on eligible fields (string, integer, number, boolean), allowing users to reference vault secrets and variables. UI schema is auto-generated by `generateVaultUischema()`. See [Vault Management](./vault-management.md) for details.
